# Integracion MCP en Pipeline MLOps con Airflow
...
3. El cliente descubre las herramientas disponibles via `list_tools()`
...
6. Cada herramienta ejecuta consultas reales contra PostgreSQL
...
| `dags/utils/mcp_server.py` | Servidor MCP con 3 herramientas que consultan PostgreSQL |
...
| Archivo | Cambio |
|---|---|
| `docker-compose.yaml` | Agregados paquetes `mcp` y `anthropic` a pip requirements |
| `.gitignore` | Agregado `.env` para proteger API keys |
...
```bash
# 1. Reiniciar contenedores (instala nuevos paquetes pip)
docker compose down
docker compose up -d
...
# 3. Ejecutar primero el pipeline ML (si no se ha ejecutado antes)
#    Activar y triggear el DAG: 04_ml_pipeline

# 4. Ejecutar el DAG de analisis MCP
...
```bash
# Crear archivo .env en la raiz del proyecto (NO se commitea)
...
En modo real:
- Claude recibe las herramientas disponibles y decide cuales usar
